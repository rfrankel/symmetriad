...
(let* ((toy-presentation (make-group-presentation '(g0 g1 g2) '()))
  ...
  (add-link-to-new-coset! toy-network 'e 'g0)
  ...
  ; TODO Add consistency checking to add-link-to-new-coset! (etc) to either forbid
...
(let* ((toy-presentation (make-group-presentation '(g0 g1 g2) '()))
       (toy-network (group-network toy-presentation #f "No name")))
  (add-link-to-new-coset! toy-network 'e 'g0)
  (add-link-to-new-coset! toy-network 'e 'g1)
...
(let* ((toy-presentation (make-group-presentation '(g0 g1 g2) '()))
  ...
  (assert-equal 'c1 (gn:product toy-network 'e 'g0))
  (assert-equal 'c2 (gn:product toy-network 'e 'g1))
  (add-link-to-new-coset! toy-network 'c2 'g2)
  ...
  ; At revision 29, identify-cosets! is called handle-coincidence! and
  ; merges the first of the two cosets it is given into the second.
  (identify-cosets! toy-network 'c3 'c1)
  (assert-equal 4 (gn:num-cosets toy-network))
...
(let* ((toy-presentation (make-group-presentation '(g0 g1 g2 g3 g4 g5) '()))
       (toy-network (group-network toy-presentation #f "No name")))
  (add-link-to-new-coset! toy-network 'e 'g0)
  (add-link-to-new-coset! toy-network 'e 'g1)
  (assert-equal 'c1 (gn:product toy-network 'e 'g0))
  ...
  (add-link-to-new-coset! toy-network 'c1 'g3)
  (add-link-to-new-coset! toy-network 'c3 'g3)
  (add-link-to-new-coset! toy-network 'c4 'g4)
  (add-link-to-new-coset! toy-network 'c5 'g4)
  (add-link-to-new-coset! toy-network 'c6 'g3)
  (add-link-to-new-coset! toy-network 'c7 'g3)
  ...
  ; off of the chains
  (add-link-to-new-coset! toy-network 'c4 'g2)
  (add-link-to-new-coset! toy-network 'c5 'g1)
  (add-link-to-new-coset! toy-network 'c8 'g0)
  (add-link-to-new-coset! toy-network 'c8 'g1)
  (add-link-to-new-coset! toy-network 'c9 'g2)
  (add-link-to-new-coset! toy-network 'c9 'g5)
  (assert-equal 'c4 (gn:product toy-network 'c10 'g2))
  (assert-equal 'c5 (gn:product toy-network 'c11 'g1))
  (assert-equal 'c8 (gn:product toy-network 'c12 'g0))
  (assert-equal 'c8 (gn:product toy-network 'c13 'g1))
  (assert-equal 'c9 (gn:product toy-network 'c14 'g2))
  (assert-equal 'c9 (gn:product toy-network 'c15 'g5))
  (assert-equal 16 (gn:num-cosets toy-network))
  (assert-equal '() (gn:dead-cosets toy-network))
  (assert-equal '(e c1 c2 c3 c4 c5 c6 c7 c8 c9 c10 c11 c12 c13 c14 c15) (gn:coset-list toy-network))
  (assert-equal 16 (gn:num-live-cosets toy-network))

  (identify-cosets! toy-network 'c1 'c3)
  (assert-equal 16 (gn:num-cosets toy-network))
  ; The merging chains
  (assert-equal '(c9 c7 c5 c3) (gn:dead-cosets toy-network))
  (assert-equal '(e c1 c2 c4 c6 c8 c10 c11 c12 c13 c14 c15) (gn:coset-list toy-network))
  (assert-equal 12 (gn:num-live-cosets toy-network))
  ; All the merged nodes lose their connections
  (assert-equal #f (gn:product toy-network 'c3 'g3))
  (assert-equal #f (gn:product toy-network 'c5 'g4))
  (assert-equal #f (gn:product toy-network 'c7 'g3))
  (assert-equal #f (gn:product toy-network 'c9 'g0))
  (assert-equal #f (gn:product toy-network 'c9 'g1))
  (assert-equal #f (gn:product toy-network 'c9 'g2))
  (assert-equal #f (gn:product toy-network 'c9 'g3))
  ...
  (assert-equal 'c4 (gn:product toy-network 'c10 'g2))
  (assert-equal 'c4 (gn:product toy-network 'c11 'g1))
  (assert-equal 'c8 (gn:product toy-network 'c12 'g0))
  (assert-equal 'c8 (gn:product toy-network 'c13 'g1))
  (assert-equal 'c8 (gn:product toy-network 'c14 'g2))
  (assert-equal 'c8 (gn:product toy-network 'c15 'g5))
  'pass)
